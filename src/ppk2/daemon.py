...
# Backward-compat aliases (existing internal callers).
_pid_path = pid_path
...
class DaemonServer:
    ...
    Uses a dedicated reader thread for serial data during measurement
    streaming (100kHz = 400KB/s requires tight polling), and ``selectors``
    for client command I/O on the Unix socket.
    """
    ...
    def __init__(self, device: PPK2Port):
        ...
        # Reader thread for serial data during streaming
    ...
    def _event_loop(self) -> None:
        """Main loop: handle client commands via selector.

        Serial data forwarding runs in a dedicated reader thread
        (started/stopped by measure_start/measure_stop commands).
        """
        while self._running:
    ...
    def _start_reader_thread(self) -> None:
        """Start a background thread that reads serial data and forwards
        to the streaming client. Runs until _state.measuring is cleared."""
        if self._reader_thread is not None:
            return

        def reader_loop():
            while self._state.measuring and self._running:
                self._forward_samples()
                # Yield briefly to avoid 100% CPU, but short enough to
                # keep up with 400KB/s serial data.
                time.sleep(0.001)

        self._reader_thread = threading.Thread(
            target=reader_loop, daemon=True, name="ppk2-serial-reader"
        )
    ...
    def _stop_reader_thread(self) -> None:
        ...
        if self._reader_thread is None:
            return
        # _state.measuring is already False — thread will exit its loop
    ...
    def _disconnect_client(self, conn: socket.socket) -> None:
        # If this was the streaming client, stop measuring and reader thread
    ...
    def _dispatch(self, conn: socket.socket, req: dict) -> None:
        ...
        try:
            if cmd == "power":
                ...
                # Include calibration data in ack so raw-mode clients
                # can parse samples locally with correct calibration.
                self._send_response(conn, {
                    "ok": True,
                    "modifiers": asdict(self._modifiers),
                    "vdd_mv": self._state.vdd_mv,
                })
                # Start reader thread for serial → client forwarding
    ...
    def _forward_samples(self) -> None:
        ...
        Called from the reader thread during streaming. Thread-safe:
        grabs a local reference to the client socket and checks
        measuring state before each operation.
        """
        raw = self._transport.read_available()
        if not raw:
            return
        ...
        if conn is None or not self._state.measuring:
            return
